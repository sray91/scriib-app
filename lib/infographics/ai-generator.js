...
export async function generateInfographicContent(request) {
  ...
  try {
    ...
    const completion = await anthropic.messages.create({
      ...
      max_tokens: 2000,
      temperature: 0.7,
      system: "You are an expert content creator specializing in infographic design. Create engaging, informative content that works well in visual format. Always respond with valid JSON.",
      messages: [
        {
          role: "user",
          content: prompt
    ...
    const content = JSON.parse(completion.content[0].text)
    return {
      success: true,
      data: content
    }
...
export async function generateAlternativeHeadlines(topic, count = 5) {
  try {
    const completion = await anthropic.messages.create({
      ...
      max_tokens: 300,
      temperature: 0.8,
      system: "You are a headline writing expert. Create catchy, engaging headlines for infographics.",
      messages: [
        {
          role: "user",
...
export async function enhanceContent(content) {
  try {
    const completion = await anthropic.messages.create({
      ...
      max_tokens: 1500,
      temperature: 0.6,
      system: "You are a content enhancement expert. Improve infographic content while maintaining the original structure and intent.",
      messages: [
        {
          role: "user",
...
Return the enhanced content in the same JSON structure.`
    ...
    return {
      success: true,
      data: JSON.parse(completion.content[0].text)
    }
  } catch (error) {
