...
class UnipileClient {
   ...
   * Start a new chat
   * @param {string} accountId - Unipile account ID
   * @param {string} profileUrl - LinkedIn profile URL or provider ID
   */
  async startChat(accountId, profileUrl) {
    // According to Unipile docs: POST /chats with account_id and attendees_ids
    return this.request(`/chats`, {
      method: 'POST',
      body: JSON.stringify({
        account_id: accountId,
        attendees_ids: [profileUrl], // Array of provider IDs
        provider: 'LINKEDIN',
      }),
    })
  }
  ...
  /**
   * Send a direct message to a LinkedIn user (creates chat if needed)
   * This combines chat creation and message sending in one API call
   * @param {string} accountId - Unipile account ID
   * @param {string} profileUrl - LinkedIn profile URL or provider ID
   * @param {string} text - Message text
   */
  async sendDirectMessage(accountId, profileUrl, text) {
    // Send message directly - Unipile will create chat if it doesn't exist
    return this.request(`/chats`, {
      method: 'POST',
      body: JSON.stringify({
        account_id: accountId,
   ...
   * According to Unipile docs, messages are fetched using the /messages endpoint
   * with chat_id as a query parameter
   */
  async getMessages(accountId, chatId, options = {}) {
    // Try the correct Unipile endpoint format: /messages with query params
  ...
  /**
   * List all posts of a user or company
   * @param {string} accountId - Unipile account ID
   * @param {string} identifier - LinkedIn user/company identifier
   * @param {object} options - Optional parameters (limit, offset, etc.)
   * @returns {Promise} List of posts with engagement metrics
   */
  async listUserPosts(accountId, identifier, options = {}) {
    const params = new URLSearchParams({
      account_id: accountId,
      ...options
    })
    return this.request(`/users/${identifier}/posts?${params}`)
  }

  /**
   * Get the authenticated account's own posts/feed
   * Alternative endpoint that might work better for fetching own posts
   * @param {string} accountId - Unipile account ID
   * @param {object} options - Optional parameters (limit, offset, etc.)
   * @returns {Promise} List of posts
   */
  async getAccountPosts(accountId, options = {}) {
    const params = new URLSearchParams({
      account_id: accountId,
      ...options
    })
  ...
  /**
   * Get feed items for the authenticated account
   * @param {string} accountId - Unipile account ID
   * @param {object} options - Optional parameters (limit, offset, etc.)
   * @returns {Promise} Feed items
   */
  async getFeed(accountId, options = {}) {
    const params = new URLSearchParams({
      account_id: accountId,
      ...options
    })
  ...
  /**
   * Retrieve a specific post with full details and metrics
   * @param {string} accountId - Unipile account ID
   * @param {string} postId - Post ID (social_id from LinkedIn)
   * @returns {Promise} Post data with analytics
   */
  async getPost(accountId, postId) {
    return this.request(`/posts/${postId}?account_id=${accountId}`)
  }

  /**
   * Create a new LinkedIn post
   * @param {string} accountId - Unipile account ID
   * @param {object} postData - Post content and configuration
   ...
   * @param {string} accountId - Unipile account ID
   * @param {string} postId - Post social_id
   * @returns {Promise} List of reactions
   */
  async listPostReactions(accountId, postId) {
    return this.request(`/posts/${postId}/reactions?account_id=${accountId}`)
  }
   ...
   */
  async listPostComments(accountId, postId) {
    return this.request(`/posts/${postId}/comments?account_id=${accountId}`)
  }
   ...
   * @param {string} accountId - Unipile account ID
   * @param {string} postId - Post social_id
   * @param {string} reactionType - Reaction type (e.g., 'LIKE', 'CELEBRATE', 'SUPPORT')
   * @returns {Promise} Reaction response
   */
  async addPostReaction(accountId, postId, reactionType = 'LIKE') {
   ...
   */
  async commentOnPost(accountId, postId, text) {
