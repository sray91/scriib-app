...
// Method 3: Using Nodemailer with custom SMTP (DISABLED - not needed for Resend setup)
// export async function sendApprovalNotificationViaNodemailer({ approverEmail, approverName, authorName, postContent, postId }) {
//   // This method is disabled since you're using Resend
//   return { success: false, error: 'Nodemailer method disabled - using Resend instead', method: 'nodemailer' }
// }
...
export async function sendApprovalNotification(params) {
  ...
  // Check if we're in development mode or if email service is not configured
  ...
  // Always add development log as fallback
