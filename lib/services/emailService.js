...
export async function sendApprovalNotificationViaNodemailer({ approverEmail, approverName, authorName, postContent, postId }) {
  ...
  try {
    ...
    if (missingVars.length > 0) {
      console.log('SMTP configuration incomplete, missing:', missingVars.join(', '))
      console.log('Would send email via Nodemailer to:', approverEmail)
      console.log('Subject:', emailTemplate.subject)
      console.log('Post ID:', postId)
      return { success: false, error: `Missing SMTP configuration: ${missingVars.join(', ')}`, method: 'nodemailer' }
    }
    ...
    return { success: true, method: 'nodemailer', email: approverEmail, messageId: info.messageId }
  } catch (error) {
